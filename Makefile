...
#
# Generated on 2026-07-16T13:03:41Z by kres d66531d.
...
integration-enterprise: integration.enterprise.test
	...
	docker run --rm --net=host --cap-drop=all --cap-add=DAC_OVERRIDE --userns=host -v /var/run:/var/run -v $(PWD)/$(ARTIFACTS)/:/out/ -v $(PWD)/$(ARTIFACTS)/integration.enterprise.test:/bin/integration.test:ro --entrypoint /bin/integration.test $(REGISTRY)/$(USERNAME)/image-factory:$(TAG) -test.v $(TEST_FLAGS) $(if $(findstring -test.image-registry,$(TEST_FLAGS)),,-test.image-registry=127.0.0.1:5000) -test.coverprofile=/out/coverage-integration-enterprise.txt -test.run $(RUN_TESTS_ENTERPRISE)
	docker rm -f local-if
	docker rm -f local-if-proxy || true
