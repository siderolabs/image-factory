...
#
# Generated on 2026-08-12T14:38:13Z by kres 76f6e99.
...
integration-direct: integration.test
	...
	$(if $(findstring -test.image-registry,$(TEST_FLAGS)),true,docker run -d -p 5000:5000 --name=local-if-proxy -e REGISTRY_PROXY_REMOTEURL=https://ghcr.io ghcr.io/distribution/distribution:edge)
	docker run --rm --net=host --cap-drop=all --cap-add=DAC_OVERRIDE --userns=host -v /var/run:/var/run -v $(PWD)/$(ARTIFACTS)/:/out/ -v $(PWD)/$(ARTIFACTS)/integration.test:/bin/integration.test:ro --entrypoint /bin/integration.test $(REGISTRY)/$(USERNAME)/image-factory:$(TAG) -test.v $(TEST_FLAGS) $(if $(findstring -test.image-registry,$(TEST_FLAGS)),,-test.image-registry=127.0.0.1:5000) -test.coverprofile=/out/coverage-integration-direct.txt -test.run $(RUN_TESTS_DIRECT)
...
integration-s3: integration.test
	...
	docker rm -f local-if-proxy || true
	$(if $(findstring -test.image-registry,$(TEST_FLAGS)),true,docker run -d -p 5000:5000 --name=local-if-proxy -e REGISTRY_PROXY_REMOTEURL=https://ghcr.io ghcr.io/distribution/distribution:edge)
	docker run --rm --net=host --cap-drop=all --cap-add=DAC_OVERRIDE --userns=host -v /var/run:/var/run -v $(PWD)/$(ARTIFACTS)/:/out/ -v $(PWD)/$(ARTIFACTS)/integration.test:/bin/integration.test:ro --entrypoint /bin/integration.test $(REGISTRY)/$(USERNAME)/image-factory:$(TAG) -test.v $(TEST_FLAGS) $(if $(findstring -test.image-registry,$(TEST_FLAGS)),,-test.image-registry=127.0.0.1:5000) -test.coverprofile=/out/coverage-integration-s3.txt -test.run $(RUN_TESTS_S3)
	docker rm -f local-if
	docker rm -f local-if-proxy || true
...
integration-cdn: integration.test
	...
	docker rm -f local-if-proxy || true
	$(if $(findstring -test.image-registry,$(TEST_FLAGS)),true,docker run -d -p 5000:5000 --name=local-if-proxy -e REGISTRY_PROXY_REMOTEURL=https://ghcr.io ghcr.io/distribution/distribution:edge)
	docker run --rm --net=host --cap-drop=all --cap-add=DAC_OVERRIDE --userns=host -v /var/run:/var/run -v $(PWD)/$(ARTIFACTS)/:/out/ -v $(PWD)/$(ARTIFACTS)/integration.test:/bin/integration.test:ro --entrypoint /bin/integration.test $(REGISTRY)/$(USERNAME)/image-factory:$(TAG) -test.v $(TEST_FLAGS) $(if $(findstring -test.image-registry,$(TEST_FLAGS)),,-test.image-registry=127.0.0.1:5000) -test.coverprofile=/out/coverage-integration-cdn.txt -test.run $(RUN_TESTS_CDN)
	docker rm -f local-if
	docker rm -f local-if-proxy || true
...
integration-proxy-installer: integration.test
	...
	docker rm -f local-if-proxy || true
	$(if $(findstring -test.image-registry,$(TEST_FLAGS)),true,docker run -d -p 5000:5000 --name=local-if-proxy -e REGISTRY_PROXY_REMOTEURL=https://ghcr.io ghcr.io/distribution/distribution:edge)
	docker run --rm --net=host --cap-drop=all --cap-add=DAC_OVERRIDE --userns=host -v /var/run:/var/run -v $(PWD)/$(ARTIFACTS)/:/out/ -v $(PWD)/$(ARTIFACTS)/integration.test:/bin/integration.test:ro --entrypoint /bin/integration.test $(REGISTRY)/$(USERNAME)/image-factory:$(TAG) -test.v $(TEST_FLAGS) $(if $(findstring -test.image-registry,$(TEST_FLAGS)),,-test.image-registry=127.0.0.1:5000) -test.coverprofile=/out/coverage-integration-direct.txt -test.run $(RUN_TESTS_PROXY)
	docker rm -f local-if
	docker rm -f local-if-proxy || true
...
integration-enterprise: integration.enterprise.test cosign
	...
	docker run --rm --net=host --cap-drop=all --cap-add=DAC_OVERRIDE --userns=host -v /var/run:/var/run -v $(PWD)/$(ARTIFACTS)/:/out/ -v $(PWD)/$(ARTIFACTS)/integration.enterprise.test:/bin/integration.test:ro --entrypoint /bin/integration.test $(REGISTRY)/$(USERNAME)/image-factory:$(TAG) -test.v $(TEST_FLAGS) -test.cosign-path=/out/cosign $(if $(findstring -test.image-registry,$(TEST_FLAGS)),,-test.image-registry=127.0.0.1:5000) -test.coverprofile=/out/coverage-integration-enterprise.txt -test.run $(RUN_TESTS_ENTERPRISE)
	docker rm -f local-if
	docker rm -f local-if-proxy || true
