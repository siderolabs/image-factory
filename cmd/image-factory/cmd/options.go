...
type ArtifactsOptions struct {
	// SPDX contains configuration for SPDX documents generated by the image factory.
...
// SPDXOptions configures the generation of SPDX documents for built images.
type SPDXOptions struct {
	// SPDXGeneratedTime configures the timestamp included in generated SPDX documents.
	// If set to "time.Now()" or "now", it will use the current time at generation.
	// Otherwise, it should be a valid RFC3339 timestamp string.
	SPDXGeneratedTime SPDXGeneratedTime `koanf:"generatedTime"`
}

// SPDXGeneratedTime configures the timestamp included in generated SPDX documents.
//
//nolint:recvcheck
type SPDXGeneratedTime struct {
	Time func() time.Time `koanf:"-"`
	Raw  string           `koanf:"-"`
}

// String returns the SPDX generated time formatted as an RFC3339 string.
func (s SPDXGeneratedTime) String() string {
	if s.Raw == "time.Now()" || s.Raw == "now" {
		return s.Raw
	}
...
// UnmarshalText implements the encoding.TextUnmarshaler interface for SPDXGeneratedTime.
func (s *SPDXGeneratedTime) UnmarshalText(text []byte) error {
	input := string(text)

	if input == "time.Now()" || input == "now" {
		s.Raw = input
		s.Time = func() time.Time { return time.Now().UTC() }
	...
	s.Time = func() time.Time { return parsedTime }

	return nil
}

// MarshalText implements the encoding.TextMarshaler interface for SPDXGeneratedTime.
func (s SPDXGeneratedTime) MarshalText() ([]byte, error) {
...
var DefaultOptions = Options{
	...
	Artifacts: ArtifactsOptions{
		...
		SPDX: SPDXOptions{
			SPDXGeneratedTime: SPDXGeneratedTime{
				Raw:  "time.Now()",
