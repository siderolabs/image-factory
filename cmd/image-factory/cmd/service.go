...
func buildEnterprisePlugins(
	...
	// Node tokens need org identity, which today only Auth0 provides; htpasswd has no
	// multi-tenancy concept to scope a token to.
	var nodeTokenVerifier enterprise.NodeTokenVerifier

	if authProvider != nil && opts.Authentication.Provider == AuthProviderAuth0 {
