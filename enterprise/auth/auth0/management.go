...
// managementSDKTestOptions lets export_test.go disable the SDK's default 5xx/429 retry
// backoff, so error-case tests don't wait through it.
var managementSDKTestOptions []auth0option.RequestOption

func newManagementSDK(ctx context.Context, tenantURL *url.URL, cfg Config) (*auth0mgmtclient.Management, error) {
	...
	opts := append([]auth0option.RequestOption{
		...
		auth0option.WithHTTPClient(&http.Client{Timeout: managementRequestTimeout}),
	}, managementSDKTestOptions...)
...
func (p *Provider) CreateNodeClient(ctx context.Context, orgID, name string) (clientID, clientSecret string, err error) {
	...
	if _, err = p.sdk.ClientGrants.Create(ctx, grant); err != nil {
		if delErr := p.sdk.Clients.Delete(ctx, created.GetClientID()); delErr != nil {
