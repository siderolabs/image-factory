...
// a Provider wired to it via IssuerURLOverride. The token endpoint is always registered;
// mux only needs the Management API routes under test.
...
// TestCreateNodeClientHappyPath pins the two requests a create issues, in order: the client
// itself, tagged with org ownership metadata, then a client-grant scoping it to the audience
// and machine scope.
...
// TestCreateNodeClientCreateFails covers the Management API rejecting the client itself,
// before any grant is attempted.
...
// TestCreateNodeClientGrantFails covers the grant call failing after the client was already
// created; the error must still surface rather than be swallowed.
...
// TestListNodeClientsFiltersByOrgAndOwner checks that clients belonging to another org, or
// not managed by image-factory at all, are excluded even though the tenant-wide list contains
// them alongside the ones that match.
func TestListNodeClientsFiltersByOrgAndOwner(t *testing.T) {
	...
	mux.HandleFunc("GET /api/v2/clients", func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "100", r.URL.Query().Get("take"))
...
// TestListNodeClientsPaginates checks the checkpoint cursor is followed across pages, since
// client_metadata cannot be filtered server-side and a real tenant list will span pages.
func TestListNodeClientsPaginates(t *testing.T) {
	...
	mux.HandleFunc("GET /api/v2/clients", func(w http.ResponseWriter, r *http.Request) {
		...
		jsonResponse(t, w, http.StatusOK, resp)
	})
...
// TestListNodeClientsRequestFails covers a failing page request surfacing as an error rather
...
// TestDeleteNodeClientRejectsInvalidClientID checks the clientID shape guard runs before any
// network call — clientID is a caller-supplied URL param and is used to build a request path.
...
// since clientID alone must never be trusted to authorize a delete.
...
// TestDeleteNodeClientLookupFails covers the lookup request itself failing, before any
// ownership check or delete is attempted.
