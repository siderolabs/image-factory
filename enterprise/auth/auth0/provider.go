...
type Provider struct {
	...
	authClient    *authentication.Authentication // nil unless browser login is configured
	logger        *zap.Logger
...
func NewProvider(ctx context.Context, logger *zap.Logger, cfg Config) (*Provider, error) {
	...
	// Browser login needs all four fields together. A partial set would otherwise
	// silently drop the /login, /callback and /logout routes, so reject it as a
	// misconfiguration. Bearer-token validation is unaffected either way.
	browserFields := map[string]bool{
	...
	for name, present := range browserFields {
		if !present {
			missing = append(missing, name)
		}
	}

	browserLogin := len(missing) == 0

	if !browserLogin && len(missing) < len(browserFields) {
		slices.Sort(missing)
	...
	// Create the authentication client for browser login; validated above as all-or-nothing.
	if browserLogin {
		...
		if cfg.HTTPClient != nil {
			authOpts = append(authOpts, authentication.WithClient(cfg.HTTPClient))
		}

		authClient, authErr := authentication.New(ctx, cfg.Domain, authOpts...)
		if authErr != nil {
