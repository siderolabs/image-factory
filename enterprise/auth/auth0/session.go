...
// The real bound is the refresh token's absolute lifetime, which Auth0 does not
// report; when the cookie outlives it the refresh fails and the user is sent to /login.
...
// state value (CSRF), the code verifier, the OIDC nonce, and the URL the user
// originally requested. It lives on the callback path and expires after 10 minutes.
...
// scoped to the callback path so it is not sent on every request.
...
// path must match the one setStateCookie used or the browser keeps the cookie.
