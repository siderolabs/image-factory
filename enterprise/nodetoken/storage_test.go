...
func TestStorageCreateListRevoke(t *testing.T) {
	...
	count, err := s.Create(ctx, "org_a", nodetoken.Record{ID: "jti-1", Name: "node-1"})
	require.NoError(t, err)
	require.Equal(t, 1, count)

	count, err = s.Create(ctx, "org_a", nodetoken.Record{ID: "jti-2", Name: "node-2"})
	require.NoError(t, err)
...
func TestStorageRevokeMissingReturnsNotFound(t *testing.T) {
	...
	_, err := s.Create(ctx, "org_a", nodetoken.Record{ID: "jti-1", Name: "node-1"})
	require.NoError(t, err)
...
func TestStorageValidCacheDelaysRevocation(t *testing.T) {
	...
	_, err := writer.Create(ctx, "org_a", nodetoken.Record{ID: "jti-1", Name: "node-1"})
	require.NoError(t, err)
