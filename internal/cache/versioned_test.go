...
func TestVersionedCache(t *testing.T) {
	...
	t.Run("fetches once and caches the result", func(t *testing.T) {
		...
		c := cache.NewVersionedCache(func(tag string) ([]string, error) {
			calls++

			return []string{tag}, nil
		})

		got, err := c.Get(context.Background(), "v1.0.0")
		require.NoError(t, err)
		assert.Equal(t, []string{"v1.0.0"}, got)
	...
	t.Run("propagates fetch errors without caching", func(t *testing.T) {
		...
		c := cache.NewVersionedCache(func(string) ([]string, error) {
			calls++
