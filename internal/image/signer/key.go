...
var (
	...
	_ BlobSigner    = (*KeySigner)(nil)
	_ ImageAttestor = (*KeySigner)(nil)
)
...
// AttestImage signs a typed in-toto statement and publishes it as a native OCI referrer.
func (s *KeySigner) AttestImage(
	ctx context.Context,
	...
	statement, err := attestation.NewStatement(subjects, predicateType, predicate)
	if err != nil {
		return fmt.Errorf("failed to build attestation statement: %w", err)
	}

	payloadType := costypes.IntotoPayloadType

	sig, err := s.sv.SignMessage(bytes.NewReader(dsse.PAE(payloadType, statement)))
	if err != nil {
		return fmt.Errorf("failed to sign attestation statement: %w", err)
	}

	keyID := s.BlobSigningIdentity()
	bundle := &protobundle.Bundle{
		MediaType: cbundle.BundleV03MediaType,
		VerificationMaterial: &protobundle.VerificationMaterial{
			Content: &protobundle.VerificationMaterial_PublicKey{
				PublicKey: &protocommon.PublicKeyIdentifier{Hint: keyID},
			},
		},
	...
	remoteOpts, err := pusher.RemoteOptions()
	if err != nil {
		return fmt.Errorf("failed to get remote options for push: %w", err)
	}

	if err = cosignremote.WriteAttestationNewBundleFormat(
		imageRef,
		bundleJSON,
		predicateType,
		cosignremote.WithRemoteOptions(append(remoteOpts, gcremote.WithContext(ctx))...),
		cosignremote.WithNameOptions(pusher.NameOptions()...),
	); err != nil {
		return fmt.Errorf("failed to push attestation referrer: %w", err)
	}

	return nil
}

// VerifyImageAttestation verifies a typed OCI referrer with the configured public key.
func (s *KeySigner) VerifyImageAttestation(
	ctx context.Context,
	imageRef name.Digest,
	predicateType string,
	...
	checkOpts.ClaimVerifier = func(sig oci.Signature, imageDigest v1.Hash, _ map[string]any) error {
